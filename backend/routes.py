...
async def create_job(request: CreateJobRequest, session: Session = Depends(get_session)):
    ...
    styles_to_use=STYLE_ORDER[:request.num_thumbnails]
    ...
    async def event_generator():
        ...
        while True:
            with Session(engine) as session:
                job=session.get(Job, job_id)  # Ensure job exists
                if not job:
                ...
                for t in thumbnails:
                    ...
                    if t.status=="completed":
                        ...
                        sent_thumbnails.add(t.id) 
                all_done=all(t.status in ("completed", "error") for t in thumbnails)
